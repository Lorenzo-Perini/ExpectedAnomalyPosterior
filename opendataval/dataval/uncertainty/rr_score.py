...
from opendataval.dataval.uncertainty.improved_prec_recall import compute_pairwise_distances

class MANIFOLD(object):
	@torch.no_grad()
	def __init__(self, real_features, fake_features, device = 'cpu'):
		...
		self.real2real_distances = compute_pairwise_distances(real_features, device = self.device)
	...
	def is_in_ball(self, k = 3, samples = None, cluster = False):
		...
		"""
		if not cluster:
			real2samples_distances = compute_pairwise_distances(self.real_features, samples, device = self.device)
		else:
			real2samples_distances = compute_pairwise_distances(self.modes, samples, device = self.device)
	...
	def rarity(self, k = 3, samples = None):
		...
		#print("out_ball_ids", out_ball_ids)
	...
	def density(self, k = 3, samples = None):
		""" The larger the real ball's size, the rare the real sample would be.
