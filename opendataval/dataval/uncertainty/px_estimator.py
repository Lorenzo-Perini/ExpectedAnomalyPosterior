...
class PxEstimator(DataEvaluator, ModelMixin):
    def __init__(
            self,
            k: int = 5,
            class_anom: int = 1,
    ...
    def train_data_values(self, *args, **kwargs):
        ...
        self.data_values = np.divide(inverse_rarity_score[len(X_tr[np.where(y_tr!=self.class_anom)[0],:]):], sum_real_inverse_rarity_scores)
        return self
