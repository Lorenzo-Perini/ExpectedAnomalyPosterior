...
class DVRL(DataEvaluator, ModelMixin):
    ...
    def input_data(
        ...
        self.num_points, [*self.feature_dim] = len(x_fakes), x_train[0].shape
    ...
    def _evaluate_baseline_models(self, *args, **kwargs):
        ...
        self.ori_model.fit(self.x_train.cpu(), self.y_train.cpu(), *args, **kwargs)

        # Trains validation model
        self.val_model = self.ori_model.clone()
        self.val_model.fit(self.x_train.cpu(), self.y_train.cpu(), *args, **kwargs)

        # Eval performance
        # Baseline performance
        y_train_hat = self.ori_model.predict(self.x_train)
        self.train_perf = self.evaluate(self.y_train, y_train_hat, metric = self.metric)
        # Compute diff
        y_pred = self.val_model.predict(self.X_extended).to(device=self.device)
        self.y_pred_diff = torch.square(self.y_extended - y_pred)
    ...
    def train_data_values(self, *args, num_workers: int = 0, **kwargs):
        ...
        kwargs : dict[str, Any], optional
            Training key word arguments
        """
        self.X_extended = torch.cat((self.x_train,self.x_fakes)).to(self.device)
        self.y_extended = torch.where(torch.cat((self.y_train,self.y_fakes)) <= 0, 0, 1).to(self.device).reshape(-1,1)
        batch_size = min(self.rl_batch_size, len(self.X_extended))
        ...
        gen = torch.Generator(self.device).manual_seed(self.random_state)
        cpu_gen = torch.Generator("cpu").manual_seed(self.random_state)
        data = CatDataset(self.x_train, self.y_train.reshape(-1,1), self.y_pred_diff[:len(self.y_train)].to(self.device))
        ...
        for j in iterator:
            x_fk_j = self.x_fakes[j,:].reshape(1,-1).to(device=self.device)
            y_fk_j = self.y_fakes[j].reshape(1,-1).to(device=self.device)
            y_fk_diff_j = self.y_pred_diff[len(self.y_train)+j].reshape(1,-1).to(device=self.device)
            for x_batch, y_batch, y_hat_batch in dataloader:
                # Moves tensors to actual device
                x_batch_ve = torch.cat((x_batch,x_fk_j)).to(device=self.device)
                y_batch_ve = torch.cat((y_batch,y_fk_j)).to(device=self.device)
                
                y_hat_batch_ve = torch.cat((y_hat_batch,y_fk_diff_j)).to(device=self.device)
                optimizer.zero_grad()

                # Generates selection probability
                
                pred_dataval = self.value_estimator(x_batch_ve, y_batch_ve, y_hat_batch_ve)
                #pred_dataval[torch.where(y_batch_ve==0)[0]] = 1
                # Samples the selection probability
                select_prob = torch.bernoulli(pred_dataval, generator=gen)
                #select_prob[torch.where(y_batch_ve==0)[0]] = 1
                ...
                new_model.fit(x_batch.cpu(), y_batch.cpu(), *args, sample_weight=select_prob.detach().cpu(), **kwargs)

                # Reward computation
                y_train_hat = new_model.predict(self.x_train)
                dvrl_perf = self.evaluate(self.y_train, y_train_hat, metric = self.metric)
                # NOTE must want to maximize the metric (IE for MSE use -MSE)
                reward_curr = dvrl_perf - self.train_perf
            ...
            data_values = self.value_estimator(x_fk_j, y_fk_j, y_fk_diff_j)
            weights = torch.cat([weights, data_values])
        weights_extended = torch.cat((torch.ones(size = (len(self.x_train),1), dtype = torch.float, device=self.device), weights))
        self.final_model = self.pred_model.clone()
        self.final_model.fit(self.X_extended.cpu(), self.y_extended.cpu(), *args, sample_weight=weights_extended.detach().cpu(), **kwargs)
    ...
    def evaluate_data_values(self) -> np.ndarray:
        ...
        """
        y_pred = self.final_model.predict(self.x_fakes).to(self.device).reshape(-1)
        y_hat = torch.square(self.y_fakes - y_pred) #torch.abs(self.y_train - y_valid_pred)
        ...
        with torch.no_grad():  # No dropout layers so no need to set to eval
            ...
            data = CatDataset(self.x_fakes, self.y_fakes.reshape(-1,1), y_hat.reshape(-1,1))
            for x_batch, y_batch, y_hat_batch in DataLoader(
...
class DveLoss(nn.Module):
    ...
    def forward(
        ...
        search_loss = (F.relu(torch.mean(pred_dataval) - self.threshold) + F.relu((1 - self.threshold) - torch.mean(pred_dataval)))
