...
class ClassifierSkLearnWrapper(Model):
    ...
    def __init__(self, base_model, num_classes: int = 2, positive_class: int = 1, *args, **kwargs):
    ...
    def predict(self, x: Union[torch.Tensor, Dataset]) -> torch.Tensor:
        ...
        output = self.model.predict_proba(x)[:,self.positive_class].reshape(-1,1)
...
class ClassifierUnweightedSkLearnWrapper(ClassifierSkLearnWrapper):
    ...
    def fit(
        ...
        **kwargs
        ...
        #print(y_train.ravel())
        self.model.fit(x_train, y_train.ravel(), *args, **kwargs)
        return self
